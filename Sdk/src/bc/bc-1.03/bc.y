...
static const char SSIRCSid[]="$Header: //depot/InterixBSD/src/bc/bc-1.03/bc.y#1 2004/09/23 05:45:17 FAREAST\\ranas $";
...
statement 		: Warranty
			...
			| expression
			    {
			      if ($1 & 2)
				bc_warn ("comparison in expression");
			      if ($1 & 1)
				generate ("W");
			      else 
				generate ("p");
			...
			| For 
			  ...
			  '(' opt_expression ';'
			    {
			      if ($4 > 1)
				bc_warn ("Comparison in first for expression");
			      $4 = next_label++;
			      if ($4 < 0)
			  ...
			  opt_expression ')'
			    {
			      if ($10 > 1)
				bc_warn ("Comparison in third for expression");
			      if ($10 < 0)
...
opt_else		: /* nothing */
			| Else 
			    {
			      bc_warn ("else clause in if statement");
			      $1 = next_label++;
			  ...
			  NEWLINE opt_auto_define_list 
			    {
                              int genbuflen = 0;
                              char* genbuf;
                              char* argbuf = arg_str($4);
                              char* autobuf = arg_str($8);
                           
			      /* Check auto list against parameter list? */
			      check_params ($4,$8);
                              /* size of buffer increased by 20 to accomodate 
                                 constant and integer portion of string, like F%d.
                               */
                              genbuflen = strlen(argbuf) + strlen(autobuf) + 20;
                              genbuf = bc_malloc(genbuflen);
			      snprintf (genbuf, genbuflen, "F%d,%s.%s[",
				       lookup($2,FUNCTDEF), 
				       argbuf, autobuf);
			      generate (genbuf);
			      free_args ($4);
...
argument_list 		: expression
			    {
			      if ($1 > 1) bc_warn ("comparison in argument");
			      $$ = nextarg (NULL,0);
			    }
			| NAME '[' ']'
			    {
...
opt_expression 		: /* empty */
			    {
			      $$ = -1;
			      bc_warn ("Missing expression in for statement");
			    }
			| expression
			;
return_expression	: /* empty */
			...
			| expression
			    {
			      if ($1 > 1)
				bc_warn ("comparison in return expresion");
			    }
			;
expression		:  named_expression ASSIGN_OP 
			  ...
			  expression
			    {
			      if ($4 > 1) bc_warn("comparison in assignment");
			      if ($2 != '=')
				{
				  sprintf (genstr, "%c", $2);
				  generate (genstr);
			...
			| NAME '(' opt_argument_list ')'
				...
				{ 
                                  int genbuflen = 0;
                                  char* genbuf;
                                  char* argbuf = call_str($3);
                                  /* size of buffer increased by 20 to accomodate
                                     constant portion and integer, like C%d,
                                   */
                                  genbuflen = strlen(argbuf) + 20;
                                  genbuf = bc_malloc(genbuflen);
				  snprintf (genbuf, genbuflen, "C%d,%s:",
					   lookup ($1,FUNCT),
					   argbuf);
				  free_args ($3);
                                  generate(genbuf);
...
named_expression	: NAME
			...
			| NAME '[' expression ']'
			    {
			      if ($3 > 1) bc_warn("comparison in subscript");
			      $$ = lookup($1,ARRAY);
			    }
			| Ibase
			    { $$ = 0; }
