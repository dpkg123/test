...
static const char SSIRCSid[]="$Header: //depot/InterixBSD/src/bc/bc-1.03/number.c#1 2004/09/23 05:45:17 FAREAST\\ranas $";
...
static bc_num
make_zero(int rscale)
{
  bc_num zero;
  if (rscale == 0)
  {
    return copy_num (_zero_);   
  }
  zero = new_num (1, rscale);
  memset (zero->n_value, 0, zero->n_len + zero->n_scale);
  return zero;
}
     ...
     bc_num n1, n2, *result;
{
  ...
  if (n1->n_sign == n2->n_sign)
	...
	case  0:
	  ...
	  sum = make_zero(MAX(n1->n_scale, n2->n_scale));
	  break;
	case  1:
     ...
     bc_num n1, n2, *result;
{
  ...
  if (n1->n_sign != n2->n_sign)
	...
	case  0:
	  /* They are equal! return zero! */
	  diff = make_zero(MAX(n1->n_scale, n2->n_scale));
	  break;
	case  1:
	  /* n2 is less than n1, subtract n2 from n1. */
	  diff = _do_sub (n1, n2);
