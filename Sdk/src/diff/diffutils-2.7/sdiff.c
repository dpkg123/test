...
static const char SSIRCSid[]="$Header: //P/src/diff/diffutils-2.7/RCS/sdiff.c 2.3 1999/04/23 18:57:05 SSI_DEV+mark Exp $";
...
#ifndef DIFF_PROGRAM
#define DIFF_PROGRAM "/bin/diff"
#endif

/* Users' editor of nonchoice */
#ifndef DEFAULT_EDITOR_PROGRAM
#define DEFAULT_EDITOR_PROGRAM "/bin/ed"
#endif
...
static int edit PARAMS((struct line_filter *, ssize_t, struct line_filter *, ssize_t, FILE*));
...
static void lf_copy PARAMS((struct line_filter *, ssize_t, FILE *));
static void lf_init PARAMS((struct line_filter *, FILE *));
static void lf_skip PARAMS((struct line_filter *, ssize_t));
...
/* This is *really* dumb, since there is a real tempnam function that */
/* does what's wanted. I think we'll use that. */
#ifndef DUMB_CODE
#define private_tempnam() tempnam (0,"dif")
#else
	...
	default:
	  try_help (0);
...
#ifdef __INTERIX_WINDOWS_WRAP
        {
            ...
            /* code used when creating program that
             * accepts Windows style filenames and then calls /
             * Interix binary in /bin
             */
            snprintf(cmdbuf, sizeof cmdbuf, "/bin/%s", __progname);
            ...
            /* set 4th arg to 0 - process all remaining args 
	     * 5th arg is command to execute
	     * - 6th arg is for additional options to be passed thru
	     */
            winwrap(argv, argc, optind, 0, cmdbuf, NULL);
...
diffarg (a)
     char const *a;
{
  static size_t diffargs, diffargsmax;
	...
	case 'e':
...

	  {
...

	    {
	      int wstatus;
#if ! HAVE_FORK
	      size_t commandLen = strlen (edbin) + strlen (tmpname) + 2;
	      char *command = xmalloc (commandLen);
	      snprintf (command, commandLen, "%s %s", edbin, tmpname);
...
private_tempnam ()
{
  ...
  bufLen = dlen + 1 + 3 + 5 + 1 + 3 + 1;
  buf = xmalloc (bufLen);

  snprintf (buf, bufLen, "%.*s/.", (int) dlen, dir);
  if (diraccess (buf))
    {
      for (index = 0;
	   ...
	   ++index)
	{
	  ...
	  snprintf (buf, bufLen, "%.*s/dif%.5lu.%c%c%c", (int) dlen,
		   dir,
