...
static const char SSIRCSid[]="$Header: /E/interix/src/diff/diffutils-2.7/context.c,v 2.1 1996/03/01 01:53:38 OpenNTBuilder Exp $";

static struct change *find_hunk(struct change *);
static void find_function(struct file_data const *, ssize_t, char const **, size_t *);
...
static void print_context_number_range(struct file_data const *, ssize_t, ssize_t);
static void print_unidiff_number_range(struct file_data const *, ssize_t,ssize_t);

/* Last place find_function started searching from.  */
static ssize_t find_function_last_search;

/* The value find_function returned when it started searching there.  */
static ssize_t find_function_last_match;

...
pr_context_hunk (hunk)
     struct change *hunk;
{
  ssize_t first0, last0, first1, last1, show_from, show_to, i;
  struct change *next;
  char const *prefix;
  char const *function;
     ...
     ssize_t a, b;
{
  ...
  /* Note: we can have B < A in the case of a range of no lines.
     ...
     which is B.  */
  if (trans_b <= trans_a)
    fprintf (outfile, trans_b == trans_a ? "%ld" : "%ld,0", trans_b);
  else
...
pr_unidiff_hunk (hunk)
     struct change *hunk;
{
  ssize_t first0, last0, first1, last1, show_from, show_to, i, j, k;
  struct change *next;
  char const *function;
  size_t function_length;
  FILE *out;
  ...
  while (script)
    {
      ...
      ssize_t first0, last0, first1, last1, deletes, inserts;
...
{
  ssize_t i = linenum;
  ssize_t last = find_function_last_search;
