...
static const char SSIRCSid[]="$Header: /E/interix/src/diff/diffutils-2.7/io.c,v 2.2 1997/01/05 21:26:28 mark Exp $";
     ...
     struct file_data *current;
{
  ...
  ssize_t alloc_lines = current->alloc_lines;
  int line = 0;
  ssize_t linbuf_base = current->linbuf_base;
  ssize_t *cureqs =xmalloc (alloc_lines * sizeof (ssize_t));
     ...
     struct file_data filevec[];
{
  ...
  ssize_t i, lines;
  size_t n0, n1, tem;
  ssize_t alloc_lines0, alloc_lines1;
  ssize_t buffered_prefix, prefix_count, prefix_mask;
  ...
  if (! (no_diff_means_no_output
	 ...
	 && filevec[1].prefix_end == p1))
    {
      ...
      while (p0 != end0)
	{
	  ssize_t l = lines++ & prefix_mask;
	  if (l == alloc_lines0)
	    linbuf0 = (char const **) xrealloc ((void*)linbuf0,
						(alloc_lines0 *= 2)
						     * sizeof(*linbuf0));
	  linbuf0[l] = p0;
	  while (*p0++ != '\n')
