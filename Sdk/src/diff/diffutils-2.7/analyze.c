...
static const char SSIRCSid[]="$Header: /E/interix/src/diff/diffutils-2.7/analyze.c,v 2.2 1997/01/05 21:16:15 mark Exp $";
...
static ssize_t *xvec, *yvec;	/* Vectors being compared. */
static ssize_t *fdiag;		/* Vector, indexed by diagonal, containing
				   1 + the X coordinate of the point furthest
				   along the given diagonal in the forward
				   search of the edit matrix. */
static ssize_t *bdiag;		/* Vector, indexed by diagonal, containing
				   the X coordinate of the point furthest
				   along the given diagonal in the backward
				   search of the edit matrix. */
static ssize_t too_expensive;	/* Edit scripts longer than this are too
...
struct partition
{
  ssize_t xmid, ymid;	/* Midpoints of this partition.  */
  int lo_minimal;	/* Nonzero if low half will be analyzed minimally.  */
  int hi_minimal;	/* Likewise for high half.  */
};

static ssize_t diag(ssize_t, ssize_t, ssize_t, ssize_t, ssize_t, struct partition *);
static struct change *add_change(ssize_t, ssize_t, ssize_t, ssize_t, struct change *);
...
static void compareseq(ssize_t, ssize_t, ssize_t, ssize_t, int);
...
diag (xoff, xlim, yoff, ylim, minimal, part)
     ssize_t xoff, xlim, yoff, ylim, minimal;
     struct partition *part;
{
  ssize_t *const fd = fdiag;	/* Give the compiler a chance. */
  ssize_t *const bd = bdiag;	/* Additional help for the compiler. */
  ssize_t const *const xv = xvec;	/* Still more help for the compiler. */
  ssize_t const *const yv = yvec;	/* And more and more . . . */
  ssize_t const dmin = xoff - ylim;	/* Minimum valid diagonal. */
  ssize_t const dmax = xlim - yoff;	/* Maximum valid diagonal. */
  ssize_t const fmid = xoff - yoff;	/* Center diagonal of top-down search. */
  ssize_t const bmid = xlim - ylim;	/* Center diagonal of bottom-up search. */
  ssize_t fmin = fmid, fmax = fmid;	/* Limits of top-down search. */
  ssize_t bmin = bmid, bmax = bmid;	/* Limits of bottom-up search. */
  ssize_t c;			/* Cost. */
  ssize_t odd = (fmid - bmid) & 1;	/* True if southeast corner is on an odd
  ...
  for (c = 1;; ++c)
	...
	{
	  ssize_t x, y, oldx, tlo = fd[d - 1], thi = fd[d + 1];
      ...
      bmin > dmin ? bd[--bmin - 1] = LONG_MAX : ++bmin;
      bmax < dmax ? bd[++bmax + 1] = LONG_MAX : --bmax;
      for (d = bmax; d >= bmin; d -= 2)
	{
	  ssize_t x, y, oldx, tlo = bd[d - 1], thi = bd[d + 1];
      ...
      if (c > 200 && big_snake && heuristic)
	{
	 ...
	 give up and report halfway between our best results so far.  */
      if (c >= too_expensive)
	{
	  ssize_t fxybest, fxbest;
	  ssize_t bxybest, bxbest;
...
static void
compareseq (ssize_t xoff,ssize_t xlim,ssize_t yoff, ssize_t ylim,int minimal)
{
  ssize_t * const xv = xvec; /* Help the compiler.  */
  ssize_t * const yv = yvec;
     ...
     struct file_data filevec[];
{
  ...
  p = xmalloc (filevec[0].equiv_max * (2 * sizeof (ssize_t)));
  equiv_count[0] = p;
  equiv_count[1] = p + filevec[0].equiv_max;
  bzero (p, filevec[0].equiv_max * (2 * sizeof (ssize_t)));
  ...
  for (f = 0; f < 2; f++)
    {
      ...
      ssize_t *counts = equiv_count[1 - f];
      ssize_t *equivs = filevec[f].equivs;
      size_t many = 5;
      size_t tem = end / 64;
  ...
  for (f = 0; f < 2; f++)
    {
      ...
      ssize_t const *equivs = filevec[f].equivs;
      ssize_t i = 0;
      ssize_t j = 0;
      ssize_t i_end = filevec[f].buffered_lines;

      while (1)
	{
	  ssize_t runlength, start, corresponding;
...
add_change (line0, line1, deleted, inserted, old)
     ssize_t line0, line1, deleted, inserted;
     struct change *old;
{
     ...
     struct file_data const filevec[];
{
  ...
  ssize_t len0 = filevec[0].buffered_lines;
  ssize_t len1 = filevec[1].buffered_lines;
     ...
     struct file_data const filevec[];
{
  ...
  char *changed0 = filevec[0].changed_flag;
  char *changed1 = filevec[1].changed_flag;
  ssize_t i0 = filevec[0].buffered_lines, i1 = filevec[1].buffered_lines;
  ...
  if (read_files (filevec, no_details_flag & ~ignore_some_changes))
  ...
  else
    {
	 ...
	 undiscarded lines.  */
      ...
      fdiag = xmalloc (diags * (2 * sizeof (ssize_t)));
      bdiag = fdiag + diags;
	...
	too_expensive <<= 1;
      ...
      compareseq (0L, filevec[0].nondiscarded_lines,
		  0L, filevec[1].nondiscarded_lines, no_discards);
	  ...
	  while (next && changes == 0)
	    {
	      ...
	      ssize_t first0, last0, first1, last1;
	      ssize_t deletes, inserts;
	...
	free (filevec[i].equivs);
      ...
      for (i = 0; i < 2; ++i)
	free ((char*)(filevec[i].linbuf + filevec[i].linbuf_base));
