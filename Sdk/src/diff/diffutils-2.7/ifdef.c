...
static const char SSIRCSid[]="$Header: /E/interix/src/diff/diffutils-2.7/ifdef.c,v 2.1 1996/03/01 01:53:38 OpenNTBuilder Exp $";
...
struct group
{
  ...
  ssize_t from, upto; /* start and limit lines for this group of lines */
};

static char *format_group PARAMS((FILE *, char *, int, struct group const *));
static char *scan_char_literal PARAMS((char *, ssize_t *));
static char *scan_printf_spec PARAMS((char *));
static ssize_t groups_letter_value PARAMS((struct group const *, int));
static void format_ifdef PARAMS((char *, ssize_t,ssize_t,ssize_t,ssize_t));
     ...
     struct group const *group;
{
  ...
  ssize_t from = group->from, upto = group->upto;
