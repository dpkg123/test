...
static const char SSIRCSid[]="$Header: /E/interix/src/diff/diffutils-2.7/diff3.c,v 2.2 1999/02/03 22:07:38 SSI_DEV+mark Exp $";
...
/* Two way diff */
struct diff_block {
  ssize_t ranges[2][2];		/* Ranges are inclusive */
...
struct diff3_block {
  ...
  ssize_t ranges[3][2];		/* Ranges are inclusive */
...
static ssize_t horizon_lines = 10;
...
static char *scan_diff_line(char *, char **, size_t *, char *, int);
static enum diff_type process_diff_control(char **, struct diff_block *);
static int compare_line_list(char * const[], size_t const[], char * const[], size_t const[],ssize_t);
static int copy_stringlist(char * const[], size_t const[], char *[], size_t[],ssize_t);
static int dotlines(FILE *, struct diff3_block *, int);
static int output_diff3_edscript(FILE *, struct diff3_block *, int const[3], int const[3], char const *, char const *, char const *);
static int output_diff3_merge(FILE *, FILE *, struct diff3_block *, int const[3], int const[3], char const *, char const *, char const *);
static size_t myread(int, char *, size_t);
static struct diff3_block *create_diff3_block(ssize_t, ssize_t,ssize_t,ssize_t,ssize_t,ssize_t);
...
static void fatal(char const *);
static void output_diff3(FILE *, struct diff3_block *, int const[3], int const[3]);
static void perror_with_exit(char const *);
static void try_help(char const *);
static void undotlines(FILE *, int,ssize_t, ssize_t);
static void usage(void);

static char *diff_program;
...
#ifdef TIMEBOMB
	if (time(0) > TIMEBOMB) {
		fprintf(stderr, "This release of OpenNT has expired.\n");
		exit(127);
	}
#endif
     ...
     struct diff3_block const *last_diff3;
{
  ...
  ssize_t lowc = D_LOWLINE (using[low_thread], FC);
  ssize_t highc = D_HIGHLINE (last_using[high_thread], FC);
  ...
  for (d = 0; d < 2; d++)
    for (ptr = using[d]; ptr; ptr = D_NEXT (ptr))
      {
	ssize_t result_offset = D_LOWLINE (ptr, FC) - lowc;
  ...
  for (d = 0; d < 2; d++)
	...
	{
	  ssize_t result_offset = D_LOWLINE (ptr, FO) - lo;
	  ssize_t linec;
  ...
  if (!using[0])
    ...
    {
      ssize_t nl0 = D_NUMLINES (result, FILE0);
      ssize_t nl1 = D_NUMLINES (result, FILE1);
...
create_diff3_block (low0, high0, low1, high1, low2, high2)
     register ssize_t low0, high0, low1, high1, low2, high2;
{
  ...
  while (scan_diff < diff_limit)
	...
	default:
	 ...
	 parcel them out among these pointers */
      if (dt != ADD)
	{
	  ssize_t numlines = D_NUMLINES (bptr, 0);
      ...
      if (dt == CHANGE)
	{
	 ...
	 parcel them out among these pointers */
      if (dt != DELETE)
	{
	  ssize_t numlines = D_NUMLINES (bptr, 1);
...
#define INT_STRLEN_BOUND(type) ((sizeof(type)*CHAR_BIT - 1) * 302 / 1000 + 2)
  ...
  *ap++ = diff_program;
  if (always_text)
    *ap++ = "-a";
  snprintf (horizon_arg, sizeof(horizon_arg), "--horizon-lines=%ld", 
	    horizon_lines);
  *ap++ = horizon_arg;
  *ap++ = "--";
  *ap++ = filea;
  ...
  *ap = 0;
  ...
  FILE *fpipe;
  size_t commandLen =   strlen(diff_program) + 30 + INT_STRLEN_BOUND (int)
      		  + 4 * (strlen (filea) + strlen (fileb));
  char *command = xmalloc (commandLen);
  char *p;
  snprintf (command, commandLen, "%s -a --horizon-lines=%ld -- ",
