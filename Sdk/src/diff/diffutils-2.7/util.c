...
static const char SSIRCSid[]="$Header: /E/interix/src/diff/diffutils-2.7/util.c,v 2.2 1999/02/03 22:07:38 SSI_DEV+mark Exp $";
...
begin_output ()
{
  ...
  nameLen =  strlen (current_name0) + strlen (current_name1)
  ...
  snprintf (name, nameLen, "diff%s %s %s", switch_string, 
  ...
  if (paginate_flag)
      ...
      if (pr_pid == 0)
	{
	  ...
	  execl (pr_path, pr_path, "-f", "-h", name, 0);
	  pfatal_with_name (pr_path);
	}
      else
	{
	  close (pipes[0]);
...
#else /* ! HAVE_FORK */
      size_t commandLen = 4 * strlen (name) + strlen (pr_path) + 10;
      char *command = xmalloc (commandLen);
      char *p;
      char const *a = name;
      snprintf (command, commandLen, "%s -f -h ", pr_path);
...
{
  ssize_t l0, l1, show_from, show_to;
  ssize_t i;
	...
	if (!ignore_blank_lines_flag || files[0].linbuf[i][0] != '\n')
	  {
	    ...
	    ssize_t len = files[0].linbuf[i + 1] - line;
	...
	if (!ignore_blank_lines_flag || files[1].linbuf[i][0] != '\n')
	  {
	    ...
	    ssize_t len = files[1].linbuf[i + 1] - line;
     ...
     char const *s1, *s2, *s3;
{
  ...
  snprintf (new, len + 1, "%s%s%s", s1, s2, s3);
  return new;
}
