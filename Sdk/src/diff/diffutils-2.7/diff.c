...
static const char SSIRCSid[]="$Header: //P/src/diff/diffutils-2.7/RCS/diff.c 2.6 1999/06/10 17:14:50 rodney Exp $";
...
static char const *filetype (struct stat const *);
static char *option_list(char **, int);
static int add_exclude_file(char const *);
static int ck_atoi(char const *, ssize_t *);
static int compare_files(char const *, char const *, char const *, char const *, int);
static int specify_format(char **, char *);
static void add_exclude(char const *);
     ...
     int count;
{
  ...
  resultLen = length + 1;
  result = xmalloc (resultLen);
  result[0] = 0;

  for (i = 0; i < count; i++)
    {
      strlcat (result, " ", resultLen);
      strlcat (result, optionvec[i], resultLen);
    }
...
#ifdef __OPENNT
  /*
   * Set up default behaviour for dircmp.
   */
  if ( strcmp(__progname, "dircmp") == 0
#ifdef __INTERIX_WINDOWS_WRAP
      || (strcmp(__progname, "dircmp.exe") == 0)
#endif
    )
    {
      IsDircmp = 1;
	...
	case 'D':
	  ...
	  specify_style (OUTPUT_IFDEF);
	  {
	    ...
	    static char const C_ifdef_group_formats[] =
	      "#ifndef %s\n%%<#endif /* not %s */\n%c#ifdef %s\n%%>#endif /* %s */\n%c%%=%c#ifndef %s\n%%<#else /* %s */\n%%>#endif /* %s */\n";
	    size_t bLen =  sizeof (C_ifdef_group_formats)
			 + 7 * strlen(optarg) - 14 	/* 7*"%s" */
			 - 8; 				/* 5*"%%" + 3*"%c" */
	    char *b = xmalloc (bLen);
	    snprintf (b, bLen, C_ifdef_group_formats,
		      optarg, optarg, 0,
		      optarg, optarg, 0, 0,
...
#define SIGCHLD SIGCLD
	...
	case 'v':
#if __OPENNT
	  printf ("%s - GNU diffutils version %s\n", program_name,
		version_string);
#else
	...
	default:
      ...
      prev = c;
        ...
        /* - setting 4th arg to 0
         *     means treat all remaining args as filenames and
         *     convert from Windows to Unix format
         * - setting 5th arg to NULL means exec() the
         *     corresponding appropriate utility.
	 * - 6th arg is for additional options to be passed thru
         */
        winwrap(argv, argc, optind, 0, NULL, NULL);

        /* NOT REACHED */

#endif /*__INTERIX_WINDOWS_WRAP */
...

  {
    ...
    ssize_t off = (width + t + GUTTER_WIDTH_MINIMUM) / (2*t)  *  t;
...
#ifdef __OPENNT
/*
 * Unfortunate bulk copy of all of the help info so the descriptive
 * lines for -d and -s can be changed and -r for dircmp mode.
 */
static char const * const dircmp_help[] = {
"-i  --ignore-case  Consider upper- and lower-case to be the same.",
"-w  --ignore-all-space  Ignore all white space.",
"-b  --ignore-space-change  Ignore changes in the amount of white space.",
"-B  --ignore-blank-lines  Ignore changes whose lines are all blank.",
...
"-N  --new-file  Treat absent files as empty.",
"-P  --unidirectional-new-file  Treat absent first files as empty.",
"-s  Don't report when two files are the same.",
...
"-d  Generate difference output for files that are different.",
"-H  --speed-large-files  Assume large files and many scattered small changes.\n",
"-v  --version  Output version info.",
"--help  Output this help.",
0
     ...
     int depth;
{
...
#ifdef __INTERIX	
  /* NT remote filesystems do not return unique inodes
   * Thus, we cannot compare inodes to see if they are the same.
   * So skip this check.
   */
  same_files = 0;
#else
  same_files = (    inf[0].desc != -1 && inf[1].desc != -1
  ...
  else if (same_files && no_diff_means_no_output)
    {
