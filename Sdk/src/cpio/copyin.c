...
#ifndef lint
static const char SSIRCSid[]="$Header: /E/interix/src/cpio/copyin.c,v 1.4 1998/09/14 19:11:48 SSI_DEV+mark Exp $";
#endif
...
process_copy_in ()
{
  ...
  while (!done)
      ...
      else
	{
	  ...
	  mode = file_hdr.c_mode & CP_IFMT;

	  uid = set_owner_flag ? set_owner : file_hdr.c_uid;
	  gid = set_group_flag ? set_group : file_hdr.c_gid;
	      ...
	      if (link_name == NULL)
		{
		  ...
		  if (!no_chown_flag) {
		    if ((chown(file_hdr.c_name, uid, gid) < 0)
			 && errno != EPERM) {
		      error (0, errno, "%s: failed to change uid,gid to %d,%d", 
				file_hdr.c_name, uid, gid);

		    }
		  }
		  /* chown may have turned off some permissions we wanted. */
		  if (chmod (file_hdr.c_name, (int) file_hdr.c_mode) < 0)
		    error (0, errno, "chmod() failed: %s", file_hdr.c_name);
...
#else
		/*
		 * OpenNT currently only supports creating FIFO files
		 * The mknod() interface is not supported
		 */
	      if (mode == CP_IFIFO) {
		  ...
		  /* don't support creating char/blk special files
		   * using mknod()
		   */
		  res = -1;
	      }
...
create_final_defers ()
{
  ...
  for (d = deferments; d != NULL; d = d->next)
	...
	error (0, errno, "%s", d->header.c_name);
      ...
      uid = set_owner_flag ? set_owner : d->header.c_uid;
      gid = set_group_flag ? set_group : d->header.c_gid;
