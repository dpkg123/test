...
#ifndef lint
static const char SSIRCSid[]="$Header: /E/interix/src/cpio/tar.c,v 1.2 1997/10/09 15:30:47 SSI_DEV+bruce Exp $";
#endif
  ...
  if (archive_format == arf_ustar)
    {
      ...
      if (name) {
	  /* 
	   * if name is bigger than the space in the hdr field, then ensure we
	   * store the right most characters i.e. truncate from the left side
	   */
	  int len = strlen(name);
	  int size = sizeof(tar_hdr->uname);
	  if (len >= size) {
	      strlcpy(tar_hdr->uname, name + (len-size-1), size);
	  } else {
	      strlcpy(tar_hdr->uname, name, size);
	  }
      }
	   ...
	   */
	  int len = strlen(name);
	  int size = sizeof(tar_hdr->gname);
	  if (len >= size) {
	      strlcpy(tar_hdr->gname, name + (len-size-1), size);
	  } else {
	      strlcpy(tar_hdr->gname, name, size);
	  }
      }
#endif
