...
#ifdef __INTERIX
int as_pack = 0;      /* running as the "pack" utility. */
int show_pack_stats = 0; /* print out stats after each file is packed. */
ulg byte_count[256];  /* keeps a count of the frequency of each byte */
#endif /* __INTERIX */
...
#ifdef __INTERIX
local void pack_usage   OF((void));
local void dump_pack_stats(void);
#endif /* __INTERIX */
...
local void usage()
{
    printf ("usage: %s [-%scdfhH?qlLnN%stvV[1-9]] [-b maxbits] [-S suffix] [file ...]\n",
	    progname,
#if O_BINARY
	    "a",
#else
    ...
    char **argv;
{
...
#ifdef __INTERIX
    else if (strncmp(progname, "pack", 4) == 0) {     /* run as "pack" */
        as_pack = 1;
    }
#endif /* __INTERIX */
...
#ifdef __INTERIX 
    /*
     * Set up the suffix as required in the "pack" case.
     */
    if (as_pack)
        z_suffix = ".z";
    else
...
#ifdef __INTERIX
    /*
     * Special handling for the pack options. We don't want
     * to introduce problems by allowing things like -r to be
     * specified.
     */
    if (as_pack) {
	int done = 0;
	...
	while ((optc = getopt_long(argc, argv, "f", 0, (int *)0))
	     != EOF) {
		switch(optc) {
		case 'f':
		    force++;
    ...
    char *iname;
{
	...
	if (as_pack) {
	    /*
	     * If running as pack, toggle the show stats flag
	     * and clear the byte_count array if necessary.
	     */
	    show_pack_stats = show_pack_stats ? 0 : 1;
	    if (show_pack_stats)
		memset(byte_count, 0, sizeof(byte_count));
	    return;
	} else {
	    /* Otherwise, accept "-" as synonym for stdin */
	    int cflag = to_stdout;
	    treat_stdin();
	    to_stdout = cflag;
	    return;
...
#ifdef __INTERIX
    /* pack is supposed to ignore empty files unless -f is specified */
    if (as_pack && istat.st_size == 0 && !force) {
	WARN((stderr, "%s: %s is empty - ignored\n",
	    progname, ifname));
	return;
    }
#endif /* __INTERIX */
...
#ifdef __INTERIX
    /*
     * The std says that the .z can be optionally specified
     * even when packing, so check for that case.
     */
    if (as_pack && errno == ENOENT) {
	char *suf;

	suf = strrchr(ifname, '.');
	if (suf != 0 && strcmp(suf, ".z") == 0) {
	    *suf = '\0';
	    if (do_stat(ifname, sbuf) == 0) return OK;
	    *suf = '.';
	}
		    ...
		    *p = (char)get_char();
			...
			error("corrupted input -- file name too large");
	            ...
	            char *base2 = base_name (base);  /* there should be problem with file name */
                    strcpy(base, base2);             /* in ofname name there can't be path */
		}
...
#ifdef __INTERIX
/* ========================================================================
 * Dump out the required statistics, including the byte_count table.
 * This routine assumes that bytes_in is properly set at this point.
 * The format of the output is indented to minimally meet standard
 * requirements. It does not match the historical output of pack.
 */
local void
dump_pack_stats()
{
    ...
    fprintf(stdout, "\tfrom %ld to %ld bytes\n", bytes_in, bytes_out);
    for (i=0; i<256; i++)
	if (byte_count[i] > 0)
		distinct++;
    fflush(stdout);

    fprintf(stdout, "\t%d distinct bytes in input\n", distinct);
    fprintf(stdout, "\theader overhead = %ld bytes\n", header_bytes);
    if (distinct) {
	fprintf(stdout, "\tDump of byte counts and relative frequency:\n\n");
	fprintf(stdout, "\tCode Count Freq  " "Code Count Freq  "
		"Byte Count Freq  " "Code Count Freq\n\t");
        for (i=0; i<256; i++) {
	    if (byte_count[i] > 0) {
    		fprintf(stdout, "%.03d  %-6ld %-.2lx%%  " , i, byte_count[i],
    		    byte_count[i]*100L/bytes_in);
		if (++count > 3) {
			fputs("\n\t", stdout);
			count = 0;
		}
